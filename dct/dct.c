/*
** http://stackoverflow.com/questions/8310749/discrete-cosine-transform-dct-implementation-c
** please set -lm option to gcc 
*/
...
int main() {
   ...
   float    
    ...
    FILE * fp = fopen("mydata.csv", "w");		// save result to "mydata.csv"
    int dimX = 8, dimY = 8;
    int i, j;
    ...
    for(i = 0; i<dimX; i++){
      for(j = 0; j<dimY; j++){
        testBlock[i][j] = testBlockB[i][j];   		// load testBlockB to testBlock
      }
    }

    begin = clock();
    dct(testDCT, testBlock, dimX, dimY);		// get DCT from testBlockB
    end = clock();
    write_mat(fp, testDCT, dimX, dimY);			//  output test result 
    
    mstime_consume = (end - begin)/(CLOCKS_PER_SEC/1000.0);
    printf("DCT consuming %f millisec.\n",mstime_consume);

    begin = clock();   
    idct(testiDCT, testDCT, dimX, dimY);		// get iDCT from DCT
    end = clock();
    write_mat(fp, testiDCT, dimX, dimY);		// output iDCT result

    mstime_consume = (end - begin)/(CLOCKS_PER_SEC/1000.0);
    printf("iDCT consuming %f millisec.\n",mstime_consume);
